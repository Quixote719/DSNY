$(document).on("click", ".completePopup", function () {
			                                  ...
			                                  TextColor = TextColors[0];
			                                   ...
			                                   TextColor = TextColors[1];
																		...
																		CustomCard += '<div id="' + item.PickupRequestId + '">\
																									 <div class="card CustomCards" style="" id="' + item.PickupRequestId + '">\
																		...
																		CustomCard += '<div class="card-block" style="padding:0px 5px 5px 5px"; >\
																 									    ...
																 									    if (item.StatusId == 1 || item.StatusId == 2) {
																 									        ...
																 									        CustomCard += '<span id="selected" style="float:right; margin-right:10px;">' + item.ReasonType + '</span></p></div></div>';
																 									    }
											CardComments = '';
	...
	$('#btnUnserviceable').click(function () {
		...
		$(document).on("click", "#btnCompleteSubmit", function () {
		    jQuery.support.cors = true;
		    var getDateUrl = APIUrl + '/ePickupsAPI/api/BulkPickups/PickupRequestsUnserviceable?pickupRequestIds='+insertData.PickupRequestId+'&reasonType='+$('input[name=gridRadios]:checked', '.form-group').val()+'&comments=&UserName='+sessionStorage.getItem("userId");
		    $.ajax({
		        async: false,
		        url: getDateUrl,
		        type: "POST",
