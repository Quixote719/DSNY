...
function validNumItems() {
    var numTV = $('#numTV').val();
    var numComps = $('#numComps').val();
    var numMon = $('#numMon').val();
    var numEKey = $('#numEKey').val();
    var numEMice = $('#numEMice').val();
    var numFax = $('#numFax').val();
    var numTVP = $('#numTVP').val();
    var numVCR = $('#numVCR').val();
    var numDVR = $('#numDVR').val();
    var numDVD = $('#numDVD').val();
    var numDCB = $('#numDCB').val();
    var numCable = $('#numCable').val();
    var numXbox = $('#numXbox').val();
    var numSSS = $('#numSSS').val();
    var numPD = $('#numPD').val();
    var numIpods = $('#numIpods').val();
    if ($('#one').prop('checked') && (numTV < 1 || numTV > 5)) {
        alert('TVs must be between 1 and 5.')
        return false;
    }
    if (numComps < 0) {
        alert('Each item selected must be greater than 0.');
        return false;
    }
    if (numMon < 0) {
        alert('Each item selected must be greater than 0.');
...
function step2Validate() {
    ...
    if (v && w && x && y && z) {
        ...
        //window.location.href = '/assets/dsny/contact/programs/ewaste/citizenPageP3.html';
        return true;
    }
}
...
function loadAddress() {
    ...
    console.log('Building Number: ' + bldg);
...
function getDates() {
    ...
    console.log("Sanitation District: " + district);

    $.getJSON( /*'https://septechjsingh.github.io/mobile_data.json'*/ 'config.json', function (data) {
        console.log(data);
        var max_dates = data.max_dates.toString();
        var get_dates_for_district = "http://54.174.123.211:3000/dates/" + district + "/" + date2Pass + "/" + max_dates;

        var settings = {
            "async": false,
            "crossDomain": true,
            "url": get_dates_for_district,
            "method": "GET",
            "headers": {
        ...
        $.ajax(settings).done(function (response) {
            ...
            for (i = 0; i < 14; i++) {
                ...
                $("<option></option>", {
                    value: encodeURI(datesArray[i].substring(0, datesArray[i].length - 9)),
                    text: datesArray[i].substring(0, datesArray[i].length - 9)
                }).appendTo('#pickupDate');
