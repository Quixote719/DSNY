function step1Validate() {
    ...
    // Check re-captcha:
    /*
    var x = document.getElementsByName('g-recaptcha-response').item(0).value;
    if (x == '' || x == null || x == undefined) {
        alert('Please verify that you are a human being by completing the Captcha.');
        return false;
    }
    */
...
function logResults(json) {
    ...
    if (returnCode == "EE") {
        ...
        for (i = 1; i <= numSimilar; i++) {
            ...
            var curr_contain = "inc_addr" + String(i);
            document.getElementById(curr_contain).style.display = "block";
        ...
        for (i = numSimilar + 1; i <= 10; i++) {
            ...
            document.getElementById(curr_contain).style.display = "none";
        }
...
function loadBoroughs() {
    $.getJSON( /*'https://septechjsingh.github.io/mobile_data.json'*/ 'config.json', function (data) {
        console.log(data);
        var num_boroughs = data.boroughs.length;
        console.log("Number of Boroughs: " + num_boroughs);
        for (i = 0; i < num_boroughs; i++) {
            var borough = data.boroughs[i].name;
            $("<option></option>", {
                value: borough,
