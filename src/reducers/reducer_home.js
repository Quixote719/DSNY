...
export default function carouselDataReducer(state = {newsData: {}}, action) {
  switch (action.type) {
    ...
    case 'SET_COLLECTION_SCHEDULE_DATA':
      return { ...state, recyclingCollectionScheduleForm: action.recyclingCollectionScheduleForm, DSNYGeoCoder: action.DSNYGeoCoder, noResultsError: action.noResultsError, suggestionAddress: action.suggestionAddress, collectionScheduleInfo: action.collectionScheduleInfo,routingData: action.routingData, collectionScheduleData: action.payload, arrayLength: action.arrayLength, holidayData: action.holidayData};
