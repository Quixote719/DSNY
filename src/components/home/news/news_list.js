    ...
    return _.map(this.props.carouselItems, item => {
      if(item.name == "news-and-updates-section"){
         return _.map(this.firstN(item.cards, 5), newsItem => {
            return (<NewsListItem description={newsItem.excerpt} title={newsItem.title} date={newsItem.date} image={newsItem.featured_image != null? newsItem.featured_image.base_path + newsItem.featured_image.file : ''} itemCounter = {itemCounter++} key={newsItem.date}/>);
        });
      }
    });
  }
